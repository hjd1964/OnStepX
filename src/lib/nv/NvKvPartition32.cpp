...
namespace {
static inline uint8_t readProgByte32_(const char* p) {
...
bool KvPartition32::initGeom_() {
  if (!vol_) return false;

  const uint16_t pb16 = vol_->partitionBlockSize(partIdx_);
  if (pb16 == 0u) return false;
  if ((pb16 & 1u) != 0u) return false; // virtual 32B blocks require even 16B count

  const uint16_t pb32 = (uint16_t)(pb16 / 2u);
  if (pb32 < 2u) return false;
  if (pb32 > MAX_BLOCKS_32) return false;

  uint32_t t = (uint32_t)pb32 * (uint32_t)KEY_TABLE_PERCENT + 500u;
  t /= 1000u;
  if (t < 1u) t = 1u;

  const uint16_t tableMax = (uint16_t)(pb32 / 2u);
  if (t > tableMax) t = tableMax;

  const uint16_t minTableForAddr8 = (pb32 > 255u) ? (uint16_t)(pb32 - 255u) : 0u;
...
KvPartition32::Status KvPartition32::init(NvVolume& vol, uint8_t partIdx) {
...
KvPartition32::Status KvPartition32::init(NvVolume& vol, uint16_t partitionName16) {
...
KvPartition32::Status KvPartition32::init(NvVolume& vol, const char* partitionAscii) {
...
KvPartition32::Status KvPartition32::readBlock32_(uint16_t block32, uint8_t out32[32]) const {
  if (!vol_) return Status::NotBound;
  if (block32 >= g_.partBlocks) return Status::OutOfRange;

  const uint16_t bo16 = block32ToBlock16Ofs_(block32);
  if (vol_->partitionReadBlock(partIdx_, bo16, out32) != NvVolume::Status::Ok) return Status::IoError;
  if (vol_->partitionReadBlock(partIdx_, (uint16_t)(bo16 + 1u), out32 + 16) != NvVolume::Status::Ok) return Status::IoError;
  return Status::Ok;
}

KvPartition32::Status KvPartition32::writeBlock32_(uint16_t block32, const uint8_t in32[32]) {
  if (!vol_) return Status::NotBound;
  if (block32 >= g_.partBlocks) return Status::OutOfRange;

  const uint16_t bo16 = block32ToBlock16Ofs_(block32);
  if (vol_->partitionWriteBlock(partIdx_, bo16, in32) != NvVolume::Status::Ok) return Status::IoError;
  if (vol_->partitionWriteBlock(partIdx_, (uint16_t)(bo16 + 1u), in32 + 16) != NvVolume::Status::Ok) return Status::IoError;
  return Status::Ok;
}

KvPartition32::Status KvPartition32::readSlot_(uint16_t slotIdx, uint8_t A[32]) {
  if (slotIdx >= g_.slotCount) return Status::OutOfRange;
  return readBlock32_(slotToBlock32Ofs_(slotIdx), A);
}

KvPartition32::Status KvPartition32::writeSlot_(uint16_t slotIdx, const uint8_t A[32]) {
  if (slotIdx >= g_.slotCount) return Status::OutOfRange;
  return writeBlock32_(slotToBlock32Ofs_(slotIdx), A);
}

KvPartition32::Status KvPartition32::readSlotName24_(uint16_t slotIdx, uint32_t& outName24) {
  if (!vol_) return Status::NotBound;
  if (slotIdx >= g_.slotCount) return Status::OutOfRange;

  const uint16_t block32 = slotToBlock32Ofs_(slotIdx);
  const uint16_t bo16 = block32ToBlock16Ofs_(block32);

  uint8_t n3[3];
  if (vol_->partitionReadInBlock(partIdx_, bo16, 0, n3, 3) != NvVolume::Status::Ok) return Status::IoError;
  outName24 = le24_(n3) & 0x00FFFFFFu;
  return Status::Ok;
}

KvPartition32::Status KvPartition32::readDataBlockByAddr_(uint8_t addr8, uint8_t out32[32]) const {
  if (!dataAddrValid_(addr8)) return Status::Corrupt;
  return readBlock32_(dataAddrToBlock_(addr8), out32);
}

KvPartition32::Status KvPartition32::writeDataBlockByAddr_(uint8_t addr8, const uint8_t in32[32]) {
  if (!dataAddrValid_(addr8)) return Status::OutOfRange;
  return writeBlock32_(dataAddrToBlock_(addr8), in32);
}

KvPartition32::Status KvPartition32::decodeEntry_(const uint8_t A[32], Decoded& out) const {
  out = Decoded{};

  out.flags2 = hdrFlags2_(A[3]);
  out.len6 = hdrLen6_(A[3]);

  const bool fInlineOnly = (out.flags2 & FLAG_INLINE_ONLY) != 0u;
  const bool fTail = (out.flags2 & FLAG_TAIL) != 0u;

  if (fInlineOnly) {
    if (fTail) return Status::Corrupt;
    if (out.len6 > INLINE_CAP) return Status::Corrupt;

    out.mode = Mode::InlineOnly;
    out.inlineLen = out.len6;
    return Status::Ok;
  }

  if (!fTail) return Status::Corrupt;
  if (out.len6 < 1u || out.len6 > BLOCK_BYTES) return Status::Corrupt;

  uint8_t body[25];
  bodyRead_(A, body);

  out.mode = Mode::AddrList;
  out.tailLen = out.len6;
  out.blockCount = 0;
...
uint16_t KvPartition32::payloadLenBytes_(const Decoded& d) const {
...
KvPartition32::Status KvPartition32::computeFullCrc24_(const uint8_t A[32], uint32_t& outCrc24) const {
  if (!vol_) return Status::NotBound;
...
KvPartition32::Status KvPartition32::validateEntryCrc_(const uint8_t A[32]) const {
...
void KvPartition32::markUsed_(uint16_t blockIdx, bool used) {
...
bool KvPartition32::isUsed_(uint16_t blockIdx) const {
...
void KvPartition32::freeEntryBlocksFromMap_(const uint8_t A[32]) {
  Decoded d;
...
KvPartition32::Status KvPartition32::findSlot_(uint32_t name24, uint16_t& outSlot, uint8_t A[32]) {
  if (!vol_) return Status::NotBound;
  name24 &= 0x00FFFFFFu;
...
KvPartition32::Status KvPartition32::findOrAllocSlot_(uint32_t name24, uint16_t& outSlot, uint8_t A[32], bool& outIsExisting) {
...
KvPartition32::Status KvPartition32::initAllocMap_() {
  memset(allocBits_, 0, sizeof(allocBits_));
...
KvPartition32::Status KvPartition32::allocDataBlocks_(uint8_t count, uint8_t outAddrs[ADDR_CAP]) {
...
KvPartition32::Status KvPartition32::get(uint32_t name24, void* dst, uint16_t dstCap, uint16_t& outLen) {
  if (!vol_) return Status::NotBound;
  name24 &= 0x00FFFFFFu;
...
KvPartition32::Status KvPartition32::put(uint32_t name24, const void* src, uint16_t len) {
  if (!vol_) return Status::NotBound;
...
KvPartition32::Status KvPartition32::del(uint32_t name24) {
  if (!vol_) return Status::NotBound;
  if (!allocInit_) return Status::Corrupt;
...
uint32_t KvPartition32::computeKey(const char* nameAscii) {
  uint32_t crc = Crc24::init();
...
uint32_t KvPartition32::computeKey(const __FlashStringHelper* nameAsciiF) {
#if defined(ARDUINO)
  ...
  for (;;) {
    const uint8_t c = readProgByte32_(p++);
    if (c == 0) break;
...
KvPartition32::Status KvPartition32::scrub(uint16_t* outDeleted) {
...
KvPartition32::Status KvPartition32::stats(Stats& out) {
