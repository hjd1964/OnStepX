...
void Axis::init(Motor *motor, void (*callback)()) {
  ...
  if (AxisSettingsSize < sizeof(AxisSettings)) { nv.protectData(true); DL("ERR: Axis::init(); AxisSettingsSize error NV subsystem writes disabled"); }
  uint16_t axesToRevert = nv.readUI(NV_AXIS_SETTINGS_REVERT);
  ...
  if (!validateAxisSettings(axisNumber, settings)) nv.protectData(true);
