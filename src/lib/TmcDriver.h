...
class TmcDriver {
  public:
    void init(int model, int8_t mosiPin, int8_t sckPin, int8_t csPin, int8_t misoPin) {
      _active = softSpi.init(mosiPin, sckPin, csPin, misoPin);
      _model = model;

      if (_model = TMC5160) _rsense = 0.075; else
      if (_model = TMC2130) _rsense = 0.11 + 0.02; else _rsense = 0.11 + 0.02;
    }
    ...
    bool setup(bool intpol, int decay_mode, byte micro_step_mode, int irun, int ihold) {
      ...
      if (_model == TMC2130) _last_chop_config=(_cc_toff<<0)+(_cc_hstart<<4)+(_cc_hend<<7)+(_cc_rndtf<<13)+(_cc_tbl<<15)+(_cc_vsense<<17)+(_cc_vhighfs<<18)+(_cc_vhighchm<<19)+(_cc_intpol<<28);
      // default=0x10410150UL
      if (_model == TMC5160) _last_chop_config=(_cc_toff<<0)+(_cc_hstart<<4)+(_cc_hend<<7)+(_cc_tbl<<15)+(_cc_vhighfs<<18)+(_cc_vhighchm<<19)+(_cc_tpfd<<20)+(_cc_intpol<<28);
