...
bool Mount::command(char *reply, char *command, char *parameter, bool *supressFrame, bool *numericReply, CommandError *commandError) {
  ...
  if (command[0] == 'h') {
    ...
    //            Return: 0 on failure
    //                    1 on success
    if (command[1] == 'P' && parameter[0] == 0) *commandError = parkGoto(); else 

    // :hQ#       Set the park position
    //            Return: 0 on failure
    //                    1 on success
    if (command[1] == 'Q' && parameter[0] == 0) *commandError = parkSet(); else 

    // :hR#       Restore parked telescope to operation
    //            Return: 0 on failure
    //                    1 on success
    if (command[1] == 'R' && parameter[0] == 0) *commandError = parkRestore(true); else return false;
  } else
