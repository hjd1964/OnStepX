...
void teleStatusFlash() {
    ...
    if (flashes == 0) { digitalWriteF(LED_STATUS_PIN, LED_STATUS_ON_STATE); return; }

    // flash the LED if there's an error
    if (cycle%2 == 0) { digitalWriteF(LED_STATUS_PIN, !LED_STATUS_ON_STATE); } else { if (cycle/2 < flashes) digitalWriteF(LED_STATUS_PIN, LED_STATUS_ON_STATE); }
  #endif
}
