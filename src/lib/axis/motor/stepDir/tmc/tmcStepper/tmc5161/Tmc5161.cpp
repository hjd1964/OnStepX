// -----------------------------------------------------------------------------------
// axis step/dir motor driver, TMC5161Stepper

#include "Tmc5161.h"
 
#if defined(DRIVER_TMC_STEPPER) && defined(TMC5161_PRESENT)
...
int StepDirTmc5161::modeMicrostepSlewing() {
  if (microstepRatio > 1) {
...
void StepDirTmc5161::modeDecayTracking() {
  setDecayMode(decay.value);
  current(iRun, iHoldRatio);
}

void StepDirTmc5161::modeDecaySlewing() {
  setDecayMode(decaySlewing.value);
  current(iGoto, 1.0F);
}

void StepDirTmc5161::readStatus() {
...
bool StepDirTmc5161::enable(bool state) {
  if (state) {
    modeDecayTracking();
  } else {
...
void StepDirTmc5161::calibrateDriver() {
...
void StepDirTmc5161::setDecayMode(int decayMode) {
