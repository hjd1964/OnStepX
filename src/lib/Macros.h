...
#define strtohostname(source) for (int i = 0; source[i]; i++) { source[i] = tolower(source[i]); if (!strchr(_allowed_fqdn "-", source[i])) { if (i < strlen(source) - 1) { strcpy(&source[i], &source[i + 1]); i--; } else source[i] = 0;; } }

// convert to lower case and strip away characters that aren't allowed in an hostname (and '-')
#define strtohostname2(source) for (int i = 0; source[i]; i++) { source[i] = tolower(source[i]); if (!strchr(_allowed_fqdn, source[i])) { if (i < strlen(source) - 1) { strcpy(&source[i], &source[i + 1]); i--; } else source[i] = 0;; } }
