...
void Site::readJD() {
  if (JulianDateSize < sizeof(ut1)) { nv.initError = true; DL("ERR: Site::readJD(); JulianDateSize error NV subsystem writes disabled"); }
  if (!nv.isKeyValid()) {
...
void Site::readLocation(uint8_t locationNumber) {
  if (LocationSize < sizeof(Location)) { nv.initError = true; DL("ERR: Site::readLocation(); LocationSize error NV subsystem writes disabled"); }
  if (!nv.isKeyValid()) {
