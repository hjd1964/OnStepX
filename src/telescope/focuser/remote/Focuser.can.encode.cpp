...
bool Focuser::encodeRequest(char *command, char *parameter) {
  ...
  if (command[0] == 'G' && command[1] == 'X') {
    // :GXU[n]#  Get focuser driver status
    if (parameter[0] == 'U' && parameter[1] >= '4' && parameter[1] <= '9' && parameter[2] == 0) {

      // figure out which CAN focuser this would logically map to
      uint8_t associatedFocuser = parameter[1] - '3'; // 1..6

      // ctrl byte: focuser number
      uint8_t ctrl = (uint8_t)(associatedFocuser & FOC_CTRL_FOCUSER_MASK);

      // check to see if the associated focuser is present
      if (!heartbeatFresh(associatedFocuser - 1)) return false;

      // if present get its driver status
      if (!beginNewRequest(FOC_OP_DRIVER_STATUS)) return false; else writeU8(ctrl);
    } else
      return false;
