...
#define CLEAN_PIN(pin) ((uint8_t)((pin) & 0xff))
  ...
  #define CLEAN_MODE_PIN(pin,mode) (((mode) == INPUT_PULLUP && CLEAN_PIN(pin) >= 34 && CLEAN_PIN(pin) <= 39) ? INPUT : (mode))
#else
  #define CLEAN_MODE_PIN(pin,mode) (mode)
#endif
  ...
  class Gpio {
    ...
    #define pinModeEx(pin,mode)       { if (pin >= 0x200) gpio.pinMode(pin-0x200,mode); else if (pin >= 0x100) pinMode(pin-0x100,mode); else if (pin >= 0) pinMode(CLEAN_PIN(pin),CLEAN_MODE_PIN(pin,mode)); }
    ...
    #define pinModeEx(pin,mode)       { if (pin >= 0x200) gpio.pinMode(pin-0x200,mode); else if (pin >= 0) pinMode(CLEAN_PIN(pin),CLEAN_MODE_PIN(pin,mode)); }
  ...
  // external GPIO analogWrite
    ...
    #define pinModeEx(pin,mode)       { if (pin > 0x100) pinMode(CLEAN_PIN(pin-0x100),mode); else if (pin >= 0) pinMode(CLEAN_PIN(pin),CLEAN_MODE_PIN(pin,mode)); }
    ...
    #define pinModeEx(pin,mode)       { if (pin >= 0) pinMode(pin,CLEAN_MODE_PIN(pin,mode)); }
