...
| `:SEO#` | `0/1` | Save absolute encoder origin, or initialize mount coordinate memory at home, when supported; see [soft-limits-disabled motion safety](STARTUP_AUTHORITY_NOTES.md#motion-safety-whenever-soft-limits-are-disabled) |
...
| `:GXSGn#` | `sg,trip,badMs,armed,latched#` | Live StallGuard telemetry for axis `n`, when supported |
